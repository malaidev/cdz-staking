...
contract('Masterdemon - Main tests', async (accounts) => {
  let masterdemon;
  let collection;
  ...
  it('[Masterdemon] General Staking Test', async () => {
    ...
    await web3.eth
      .getBalance(masterdemon.address)
      .then((res) => (balanceBefore = web3.utils.fromWei(res, 'ether')));
    await masterdemon.stake(0, 0, {
      from: accounts[0],
      value: web3.utils.toWei('0.01', 'ether'),
    });
    await masterdemon.batchStake(0, [1, 2, 3], {
      from: accounts[0],
      value: web3.utils.toWei('0.03', 'ether'),
    });
    await web3.eth
      .getBalance(masterdemon.address)
      .then((res) => (balanceAfter = web3.utils.fromWei(res, 'ether')));
    assert.equal(Number((balanceAfter - balanceBefore).toFixed(2)), 0.04);

    let stakedAmount;
    await masterdemon
      .getUserInfo(accounts[0], collection.address)
      .then((res) => {
  ...
  it('[Masterdemon] General Unstaking Test', async () => {
    ...
    await masterdemon.batchStake(0, [1, 2, 3, 4, 5], {
      from: accounts[0],
    ...
    await masterdemon.batchStake(0, [10, 11, 12], {
      from: accounts[2],
      value: web3.utils.toWei('0.05', 'ether'),
    });
    ...
    await truffleAssert.fails(
      masterdemon.unstake(0, 11, { from: accounts[1] }),
      truffleAssert.ErrorType.REVERT,
    );

    assert.equal(amountOfStakers, 2);

    await masterdemon.batchUnstake(0, [10, 11, 12], { from: accounts[2] });
    await masterdemon.getCollectionInfo(0).then((res) => {
      amountOfStakers = res['4'].words[0];
    });
  ...
  it('[Masterdemon] Admin Actions Test', async () => {
